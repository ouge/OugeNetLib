...
void Connector::connect() {
    ...
    switch (savedErrno) {
        ...
        case EINPROGRESS:
        case EINTR:
        case EISCONN: connecting(sockfd); break;

        case EAGAIN:
        case EADDRINUSE:
        case EADDRNOTAVAIL:
        case ECONNREFUSED:
        case ENETUNREACH: retry(sockfd); break;
        ...
        case EFAULT:
        case ENOTSOCK:
            std::cerr << "connect error in Connector::startInLoop "
                      << savedErrno;
            sockets::close(sockfd);
            break;

        default:
            std::cerr << "Unexpected error in Connector::startInLoop "
                      << savedErrno;
            sockets::close(sockfd);
...
void Connector::handleWrite() {
    ...
    if (state_ == kConnecting) {
        ...
        if (err) {
            std::cerr << "Connector::handleWrite - SO_ERROR = " << err << " "
                      << strerror(err);
            retry(sockfd);
        } else if (sockets::isSelfConnect(sockfd)) {
            std::cerr << "Connector::handleWrite - Self connect";
            retry(sockfd);
        } else {
            setState(kConnected);
            if (connect_) {
